...
function Estatisticas() {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                const resTotalVotos = await axios.get('http://localhost:3001/votes/total');
                const resVotosPorHora = await axios.get('http://localhost:3001/votes/history');

                const resVotosPorParticipanteA = await axios.get('http://localhost:3001/votes/participante/participanteA');
                const resVotosPorParticipanteB = await axios.get('http://localhost:3001/votes/participante/participanteB');

                const sortedData = resVotosPorHora.data.sort((a, b) => a._id.hour - b._id.hour);

                const resA = await axios.get('http://localhost:3001/votes/hourly/participanteA');
                const resB = await axios.get('http://localhost:3001/votes/hourly/participanteB');
        ...
        socket.onmessage = (event) => {
            ...
            if (data.type === 'newVote') {
                ...
                // Refetch os dados após um novo voto
                fetchData();
            }
        };
